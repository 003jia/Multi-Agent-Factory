...
import { tierToModel } from "../modelRouter.js";

interface ClaudeArtifactResponse {
  files: GeneratedFile[];
  agentNotes: string;
...
export class ClaudeEngine implements AgentEngine {
  ...
  readonly models: Record<"economy" | "quality", string>;
  readonly baseUrl?: string;
  private client: Anthropic;

  constructor(apiKey: string, models?: Record<"economy" | "quality", string>, baseUrl?: string) {
    this.models = models ?? { economy: tierToModel("economy"), quality: tierToModel("quality") };
  ...
  async generateRequirements(task: Task): Promise<string> {
    return this.textCompletion({
      task,
      modelTier: task.complexity >= 6 ? "quality" : "economy",
      prompt: `请为以下任务生成中文需求文档，包含背景、目标、范围、流程、验收标准，保持可供用户确认和编辑。\n\n任务标题：${task.title}\n任务描述：${task.prompt}`
    });
  }
  ...
  async regenerateRequirements(task: Task, feedback: string, previousContent: string): Promise<string> {
    return this.textCompletion({
      task,
      modelTier: task.complexity >= 6 ? "quality" : "economy",
      prompt: `请根据用户反馈重写需求文档，保留合理内容并明确响应反馈。\n\n用户反馈：${feedback}\n\n上一版：\n${previousContent}`
    });
  }
  ...
  async generatePlan(task: Task, requirements: string): Promise<string> {
    return this.textCompletion({
      task,
      modelTier: task.complexity >= 6 ? "quality" : "economy",
      prompt: `请基于已确认需求生成任务计划，包含阶段、任务拆分、agent 分派依据、验证标准。\n\n任务：${task.title}\n\n需求：\n${requirements}`
    });
  }
  ...
  async regeneratePlan(task: Task, feedback: string, previousContent: string): Promise<string> {
    return this.textCompletion({
      task,
      modelTier: task.complexity >= 6 ? "quality" : "economy",
      prompt: `请根据用户反馈重写任务计划，保持可执行、可验证。\n\n用户反馈：${feedback}\n\n上一版：\n${previousContent}`
    });
  }

  async splitWorkItems(task: Task, plan: string): Promise<WorkItem[]> {
    const payload = await this.jsonCompletion<{ items: Array<Omit<WorkItem, "id" | "taskId" | "status" | "assignedSubagentId">> }>({
      task,
      modelTier: task.complexity >= 7 ? "quality" : "economy",
      prompt: `请把任务计划拆分为 3-6 个 work item。只返回 JSON：{"items":[{"title":"","description":"","complexity":1-10,"preferredModelTier":"economy|quality"}]}。\n\n任务：${task.title}\n计划：\n${plan}`
    });
  ...
  async generateArtifact(
    ...
    const fixBlock = errorContext?.trim()
      ...
      : "";
    const payload = await this.jsonCompletion<ClaudeArtifactResponse>({
      task,
      modelTier: assignment.modelTier,
      stream: true,
      prompt: `你是 ${subagent.name}，角色：${subagent.role}，能力：${subagent.skills.join("、")}。
...
work item 描述：${workItem.description}`
    ...
    return {
      ...
      files: payload.files.map((file) => ({
        ...
        summary: file.summary || "Claude 生成文件",
  ...
  async reviewArtifact(
    ...
    const payload = await this.jsonCompletion<ClaudeReviewResponse>({
      task: {
        id: artifact.taskId,
        projectId: null,
        title: workItem.title,
        prompt: workItem.description,
        selectedFiles: workItem.targetFiles,
        constraints: workItem.acceptanceChecks.join("\n"),
      ...
      modelTier: "quality",
      prompt: `你是代码审查 Agent。请审查以下生成产物，只返回 JSON：
...
${artifact.files.map((file) => `### ${file.path}\n${file.content}`).join("\n\n")}`
    ...
    return payload.findings.map((finding) => ({
      ...
      source: "model",
      message: finding.message || "Claude 审查完成。",
      suggestedFix: finding.suggestedFix || "无需修改。",
      createdAt: now()
    }));
  }

  private async textCompletion(input: { task: Task; modelTier: "economy" | "quality"; prompt: string }): Promise<string> {
    const message = await this.createMessage(input);
    return textFromMessage(message);
  }

  private async jsonCompletion<T>(input: {
    task: Task;
    modelTier: "economy" | "quality";
    prompt: string;
    stream?: boolean;
  }): Promise<T> {
    const message = input.stream ? await this.streamMessage(input) : await this.createMessage(input);
    const text = textFromMessage(message);
    return parseJson<T>(text);
  }

  private createMessage(input: { task: Task; modelTier: "economy" | "quality"; prompt: string }) {
    const request: any = {
      model: this.models[input.modelTier],
      max_tokens: input.modelTier === "quality" ? 4096 : 2048,
      system: systemPrompt,
      messages: [{ role: "user", content: input.prompt }]
    };
    if (input.modelTier === "quality") request.thinking = { type: "adaptive" };
    return this.client.messages.create(request);
  }

  private async streamMessage(input: { task: Task; modelTier: "economy" | "quality"; prompt: string }) {
    const request: any = {
      model: this.models[input.modelTier],
      max_tokens: input.modelTier === "quality" ? 8192 : 4096,
      system: systemPrompt,
      messages: [{ role: "user", content: input.prompt }]
    };
    if (input.modelTier === "quality") request.thinking = { type: "adaptive" };
...
function parseJson<T>(text: string): T {
  ...
  try {
    ...
    throw new Error(`Claude 返回内容不是合法 JSON：${error instanceof Error ? error.message : "解析失败"}`);
  }
}
